document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CONFIGURATION ---
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzCKTnlvLstibW3S22dB1IlEqxpB8SNCVrjATaA_CXlAlazQeMYimpSveHkQGwM2H6e-w/exec'; // Use your latest deployment URL
    const STUDENT_DOMAIN = '@student.annauniv.edu';
    const DISCOUNT_PERCENTAGE = 0.75; // 70% discount
    ...
    // --- HELPER FUNCTION (Reverted to the old, working version) ---
    const sendData = (data) => {
        data.timestamp = new Date().toISOString();
        // This 'no-cors' mode is required to avoid the browser's preflight security check,
        // which Google's script server does not handle.
        fetch(SCRIPT_URL, {
            method: 'POST',
            mode: 'no-cors', // This is the critical line from your old script
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify(data)
        }).catch(err => console.error('Error sending final data:', err)); // We can still see errors if the request fails to send
    };
